...
import { CheckCircle2, BookOpen, NotebookPen, MessageCircle, Flame } from "lucide-react";
...
export default async function PerfilPage() {
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-serif text-stone-800">Perfil</h1>
        <p className="text-stone-500 text-sm mt-1">Sua identidade, ciclo e configurações</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        {[
          { label: "Sessões", value: sessionCount, icon: MessageCircle, color: "text-rose-500" },
          { label: "Diário", value: journalCount, icon: NotebookPen, color: "text-teal-500" },
          { label: "Módulos", value: moduleCount, icon: BookOpen, color: "text-amber-600" },
          { label: "Assessments", value: assessmentCount, icon: CheckCircle2, color: "text-purple-500" },
            ...
            {milestones.map((m) => (
              ...
              >
                ...
                <p className="text-xs text-stone-300 shrink-0">
                  {new Date(m.achievedAt).toLocaleDateString("pt-BR", {
                    day: "numeric",
                    month: "short",
                  })}
                </p>

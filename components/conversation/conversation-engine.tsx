...
export default function ConversationEngine({ initial }: { initial: Convo[] }) {
  ...
  const CLOSE_ICON: Record<string, React.ReactNode> = {
    closer: <ArrowRight className="h-3.5 w-3.5" style={{ color: "#6e7b61" }} />,
    distant: <ArrowDown className="h-3.5 w-3.5 text-rose-500" />,
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      <div className="mb-6 flex items-center gap-2.5">
        <MessagesSquare className="h-6 w-6" style={{ color: "#6d4a5a" }} />
        <div>
      ...
      <div className="rounded-xl border border-stone-200 bg-white p-5 mb-8">
        <p className="text-[11px] uppercase tracking-[0.16em] font-semibold mb-3" style={{ color: "#6d4a5a" }}>
          {t("prepareTitle")}
        </p>
        ...
        <div className="space-y-4">
          {PREP.map((k, i) => (
            <div key={k} className={i === 3 ? "rounded-lg border-l-2 border-amber-400 pl-4" : ""}>
              <label className={i === 3 ? "text-sm font-medium text-amber-800" : "text-sm font-medium text-stone-700"}>
                {t(`prep_${k}`)}
              </label>
              <textarea
                value={prep[k]}
            ...
            <div key={c.id} className="rounded-xl border border-stone-200 bg-white p-4">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  {c.gardenKey && (
                    <p className="text-[11px] mt-0.5" style={{ color: "#6e7b61" }}>
                      {tg(`beds.${c.gardenKey}`)}
                    </p>
                  )}
                </div>
                <span
                  ...
                  style={
                    c.status === "DONE"
                      ? { background: "rgba(110,123,97,0.15)", color: "#5b6b4e" }
